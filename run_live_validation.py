...
@st.cache_data(ttl=45,show_spinner=False,max_entries=4)
def discovery_snapshot(market:str):
    """KIS 엔진의 실시간 순위/급등 후보를 합쳐 넓은 1차 후보군을 만든다.

    고정 KR/US_UNIVERSE는 API가 후보를 전혀 주지 못할 때만 안전망으로 사용한다.
    """
    ranked={}
    modes=("국내 돌파","국내 거래대금 급증") if market=="국내" else ("미국 30분 1% 타점","미국 급등주")
    raw_count=0
    for mode in modes:
        try:
            rows=scanner().candidates(mode) or []
            for row in rows:
                raw_count+=1
                c=_compact_discovery_row(row,market); ticker=c.get("ticker","")
    ...
    if not stage1:
        ...
        stage1=[dict(r,screen_price=0,screen_volume=0,screen_change=0,screen_value=0,discovery_score=0) for r in source[:12]]
...
def latest_entry_candidates(market:str,minimum_score:float,limit:int=5):
    ...
    for ticker,name,issued,price,verdict,score,entry_ok,data_valid,f5,f10,f20,f30,detail_json in rows:
        ...
        if not (data_valid and d.get("level_plan_valid") and d.get("repeat_candidate") and rr>=1.2 and 0.5<=width<=1.5 and (spread is None or spread<=(0.35 if code=="KR" else 0.25))): continue
