...
def connect() -> sqlite3.Connection:
    ...
    db.execute("""
        CREATE TABLE IF NOT EXISTS signals (
            ...
            max_up5 REAL, max_down5 REAL, max_up10 REAL, max_down10 REAL,
            max_up20 REAL, max_down20 REAL, max_up30 REAL, max_down30 REAL,
            result_done INTEGER NOT NULL DEFAULT 0,
            stop_price REAL, data_valid INTEGER NOT NULL DEFAULT 0,
            hit05_before_stop INTEGER, hit10_before_stop INTEGER, hit15_before_stop INTEGER,
    ...
    migrations = {
        "stop_price": "REAL", "data_valid": "INTEGER NOT NULL DEFAULT 0",
        "max_up5": "REAL", "max_down5": "REAL",
        "max_up10": "REAL", "max_down10": "REAL",
        "max_up20": "REAL", "max_down20": "REAL",
        "max_up30": "REAL", "max_down30": "REAL",
        "hit05_before_stop": "INTEGER", "hit10_before_stop": "INTEGER",
        "hit15_before_stop": "INTEGER",
...
def grade_pending(db: sqlite3.Connection, now_ts: int) -> None:
    ...
    for signal_id, market, ticker, issued, base, a5, a10, a20, a30, stop_price in rows:
        ...
        for minutes, existing in ((5, a5), (10, a10), (20, a20), (30, a30)):
            ...
            if now_ts >= issued + minutes * 60 and base > 0:
                extrema = db.execute("""
                    SELECT MAX(price),MIN(price) FROM quotes
                    WHERE market=? AND ticker=? AND captured BETWEEN ? AND ?
                """, (market, ticker, issued, issued + minutes * 60)).fetchone()
                if extrema and extrema[0] is not None:
                    updates[f"max_up{minutes}"] = (f(extrema[0]) / base - 1) * 100
                    updates[f"max_down{minutes}"] = (f(extrema[1]) / base - 1) * 100
        if now_ts >= issued + 30 * 60:
            ...
            for column, goal in (("hit05_before_stop", 0.5),
                                 ("hit10_before_stop", 1.0),
                                 ("hit15_before_stop", 1.5)):
                target = base * (1 + goal / 100)
                target_index = next((index for index, (_, price) in enumerate(path) if f(price) >= target), None)
                hit = target_index is not None and (stop_index is None or target_index < stop_index)
                updates[column] = int(hit)
            projected = {
...
def export_summary(db: sqlite3.Connection) -> None:
    rows = db.execute("""
        SELECT market,ticker,name,datetime(issued,'unixepoch','+9 hours'),base_price,
               ...
               forecast20,actual20,forecast30,actual30,
               max_up5,max_down5,max_up10,max_down10,max_up20,max_down20,max_up30,max_down30,
               stop_price,data_valid,hit05_before_stop,hit10_before_stop,hit15_before_stop,
               hit1_before_stop,hit2_before_stop,hit3_before_stop,stop_first,detail_json
        FROM signals ORDER BY issued DESC
    """).fetchall()
    headers = [
        "시장", "티커", "종목명", "신호시각(KST)", "기준가", "판정", "점수", "진입가능",
        "예상5분", "실제5분", "예상10분", "실제10분", "예상20분", "실제20분",
        "예상30분", "실제30분", "5분최대상승", "5분최대하락", "10분최대상승",
        "10분최대하락", "20분최대상승", "20분최대하락", "30분최대상승", "30분최대하락",
        "손절가", "데이터유효", "+0.5%선도달", "+1.0%선도달", "+1.5%선도달",
        "+1%선도달(호환)", "+2%선도달", "+3%선도달", "손절선도달", "판정원자료JSON",
    ]
    with CSV_PATH.open("w", newline="", encoding="utf-8-sig") as handle:
        writer = csv.writer(handle)
        writer.writerow(headers)
        writer.writerows(rows)
    export_html_report_v2(db)
...
def export_html_report_v2(db: sqlite3.Connection) -> None:
    """Write a complete, audit-friendly report from raw validation observations."""
    report_path = DB_PATH.parent / "validation_report.html"
    columns = [
        "market", "ticker", "name", "issued", "base_price", "verdict", "score", "entry_ok",
        "data_valid", "forecast5", "actual5", "forecast10", "actual10", "forecast20", "actual20",
        "forecast30", "actual30", "max_up5", "max_down5", "max_up10", "max_down10",
        "max_up20", "max_down20", "max_up30", "max_down30", "hit05_before_stop",
        "hit10_before_stop", "hit15_before_stop", "stop_first", "detail_json",
    ]
    raw = db.execute(f"SELECT {','.join(columns)} FROM signals WHERE result_done=1 ORDER BY issued DESC").fetchall()
    records = [dict(zip(columns, row)) for row in raw]
    valid = [row for row in records if row["data_valid"] == 1]
    entries = [row for row in valid if row["entry_ok"] == 1]

    def judged_direction(rows, minutes):
        forecast, actual = f"forecast{minutes}", f"actual{minutes}"
        return [((f(row[forecast]) >= 0) == (f(row[actual]) >= 0)) for row in rows if row[actual] is not None]

    def metric_card(label, values):
        interval = wilson_interval(values)
        ci = f"95% 범위 {interval[0]*100:.1f}~{interval[1]*100:.1f}%" if interval else "신뢰구간 계산 전"
        return (f'<div class="card"><small>{html.escape(label)}</small><b>{ratio(values)}</b>'
                f'<span>표본 {len(values)}건</span><span>{ci}</span></div>')

    cards = []
    for minutes in (5, 10, 20, 30):
        cards.append(metric_card(f"전체 유효신호 {minutes}분 방향 적중", judged_direction(valid, minutes)))
        cards.append(metric_card(f"진입가능만 {minutes}분 방향 적중", judged_direction(entries, minutes)))
    for label, field in (("진입가능 +0.5% 선도달", "hit05_before_stop"),
                         ("진입가능 +1.0% 선도달", "hit10_before_stop"),
                         ("진입가능 +1.5% 선도달", "hit15_before_stop")):
        cards.append(metric_card(label, [bool(row[field]) for row in entries if row[field] is not None]))

    def mean(rows, field):
        values = [f(row[field]) for row in rows if row[field] is not None]
        return f"{sum(values) / len(values):+.3f}%" if values else "표본 없음"

    extrema_cards = "".join(
        f'<div class="card"><small>{label}</small><b>{value}</b><span>진입가능 신호 기준</span></div>'
        for label, value in (
            ("30분 평균 최대상승", mean(entries, "max_up30")),
            ("30분 평균 최대하락", mean(entries, "max_down30")),
        )
    )
    ...
    def row_html(row):
        ...
        values = [
            ...
            datetime.fromtimestamp(row["issued"], KST).strftime("%Y-%m-%d %H:%M:%S"),
            f(row["base_price"]), row["verdict"], row["score"],
            "진입가능" if row["entry_ok"] else "관찰", "유효" if row["data_valid"] else "제외",
            row["hit05_before_stop"], row["hit10_before_stop"], row["hit15_before_stop"],
            row["actual5"], row["actual10"], row["actual20"], row["actual30"],
            row["max_up30"], row["max_down30"], row["stop_first"], detail,
        ]
        return "<tr>" + "".join(f"<td>{html.escape(str(v if v is not None else '-'))}</td>" for v in values) + "</tr>"

    successes = sorted(entries, key=lambda row: f(row["max_up30"], -999), reverse=True)[:20]
    failures = sorted(entries, key=lambda row: f(row["max_down30"], 999))[:20]
    all_rows = "".join(row_html(row) for row in records[:500])
    success_rows = "".join(row_html(row) for row in successes)
    failure_rows = "".join(row_html(row) for row in failures)
    headers = ["시장", "티커", "종목명", "시각(KST)", "기준가", "판정", "점수", "구분", "데이터",
               "+0.5%", "+1.0%", "+1.5%", "5분", "10분", "20분", "30분", "최대상승", "최대하락",
               "손절먼저", "원자료"]
    header_html = "".join(f"<th>{header}</th>" for header in headers)
    table = lambda body: f"<table><thead><tr>{header_html}</tr></thead><tbody>{body}</tbody></table>"
    document = f"""<!doctype html><html lang='ko'><meta charset='utf-8'><title>반복단타 자동검증 보고서</title>
    <style>body{{font-family:Arial,sans-serif;max-width:1500px;margin:30px auto;padding:0 16px;color:#20242c}}
    .cards{{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:12px}}.card{{background:#f4f6fa;border-radius:12px;padding:16px}}
    .card b,.card span{{display:block}}.card b{{font-size:27px;margin:8px 0}}.warn{{background:#fff4dc;padding:15px;border-radius:10px;margin:18px 0}}
    .scroll{{overflow:auto;max-height:620px}}table{{border-collapse:collapse;width:100%;font-size:12px}}th,td{{border-bottom:1px solid #ddd;padding:7px;text-align:left;white-space:nowrap}}</style>
    <h1>반복단타 자동검증 보고서</h1><p>완료 {len(records)}건 · 유효 {len(valid)}건 · 진입가능 {len(entries)}건</p>
    <div class='warn'>수치는 저장된 실제 시세 경로의 사후채점입니다. 수익을 보장하지 않으며 표본 수와 신뢰구간을 함께 확인해야 합니다.</div>
    <div class='cards'>{''.join(cards)}{extrema_cards}</div>
    <h2>대표 성공 사례</h2><div class='scroll'>{table(success_rows)}</div>
    <h2>대표 실패 사례</h2><div class='scroll'>{table(failure_rows)}</div>
    <h2>전체 종목별 기록 (최근 500건)</h2><div class='scroll'>{table(all_rows)}</div></html>"""
    report_path.write_text(document, encoding="utf-8")
