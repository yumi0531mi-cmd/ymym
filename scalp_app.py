...
def live_filtered_universe(market: str) -> list[dict]:
    ...
    for mode in modes:
        try:
            for row in scanner().candidates(mode):
                ...
                merged["screen_volume"] = max(int(candidate.get("screen_volume") or 0), int(old.get("screen_volume") or 0))
                ranked_rows[ticker] = merged
        except Exception:
            pass

    for row in ranked_rows.values():
        p = float(row.get("screen_price", 0) or 0)
        c = float(row.get("screen_change", 0) or 0)
        v = float(row.get("screen_volume", 0) or 0)
        if p > 0 and p <= limit and c >= 0.0 and v >= 1000:
            accepted.append(row)

    accepted.sort(
        key=lambda x: (
            float(x.get("screen_change", 0) or 0),
            float(x.get("screen_volume", 0) or 0),
        ),
        reverse=True,
    )
    if accepted:
    ...
    for item in source:
        accepted.append({**item, "screen_price": 0.0, "screen_change": 0.0, "screen_volume": 0})
    return accepted[:15]
