...
def load_secret(name):
    try:
        ...
        return ""
        # ==============================================================================
# [신규 추가] 한국투자증권(KIS) 전용 호가 반영 및 초단타 계산 모듈
# ==============================================================================


def adjust_krx_tick_size(price):
    """국내주식 호가 단위(Tick Size) 자동 정렬 (2023 개정 기준)"""
    if price < 2000:
        return round(price)
    elif price < 5000:
...
def calculate_scalping_targets(
    ...
    """1M / 3M / 5M / 15M 분봉 가격 기준 타점 자동 계산"""
    try:
        current_price = float(current_price)
        low_1m = float(low_1m)
        low_3m = float(low_3m)
        high_5m = float(high_5m)
        high_15m = float(high_15m)
        if holding_entry_price:
            holding_entry_price = float(holding_entry_price)
    except (ValueError, TypeError):
        return None

    # 1. 진입가 결정 (보유 중이면 진입가 고정)
    entry_price = (
        holding_entry_price
    ...
    # 2. 손절가 (1M/3M 최저점 중 높은 값과 -1.8% 비율 손절 중 높은 가격)
    structural_stop = max(low_1m, low_3m)
    percent_stop = entry_price * 0.982
    stop_loss = max(structural_stop, percent_stop)

    risk = entry_price - stop_loss
    if risk <= 0:  # 손절선 예외 안전장치
        risk = entry_price * 0.015
        stop_loss = entry_price - risk

    # 3. 1차 익절가 (Risk/Reward 1:1.5 및 5분봉 전고점 반영)
    rr_target_1 = entry_price + (risk * 1.5)
    target_1 = (
        min(high_5m, rr_target_1) if high_5m > entry_price else rr_target_1
    )

    # 4. 2차 익절가 (Risk/Reward 1:3.0 및 15분봉 전고점 반영)
    rr_target_2 = entry_price + (risk * 3.0)
    target_2 = max(high_15m, rr_target_2)

    # 한국 주식일 경우 호가 단위(Tick) 정렬
    if not is_us_stock:
        entry_price = adjust_krx_tick_size(entry_price)
        stop_loss = adjust_krx_tick_size(stop_loss)
        target_1 = adjust_krx_tick_size(target_1)
        target_2 = adjust_krx_tick_size(target_2)
...
def render_scalping_card(
    ...
    """기존 CSS 스타일을 이용해 완벽하게 종목 카드를 출력하는 함수"""
    is_us_stock = market.upper() in ["NAS", "NYS", "AMS", "US", "NASDAQ", "NYSE"]
    ...
    if not calc:
        st.error(f"[{ticker}] 가격 데이터 수신 오류")
        return
    ...
    # 가격 포맷 설정 (미국: 소수점 2~3자리 / 한국: 정수)
    fmt = (
        (lambda x: f"{x:,.2f}" if x >= 100 else f"{x:,.3f}")
        if is_us_stock
        else (lambda x: f"{int(x):,}")
    )
    ...
    # 상태별 판정
    if in_position:
        pnl_pct = ((cur_p - entry) / entry) * 100
        if cur_p <= stop:
            verdict_class = "v-red"
            verdict_text = f"🚨 손절 대응 필요! (현재가 {curr_symbol}{fmt(cur_p)} ≤ 손절가 {curr_symbol}{fmt(stop)})"
        elif cur_p >= t2:
    ...
    # 기존 올려주신 CSS 구조와 100% 동일한 HTML 생성
    html_markup = f"""
    <div class="stock-card">
        <div class="card-head">
            <div>
                <div class="stock-name">{html.escape(str(name))} <span style="font-size:0.75rem; color:#8b95a8;">({market})</span></div>
                <div class="ticker">{html.escape(str(ticker))}</div>
            </div>
            <div>
                <div class="price">{curr_symbol}{fmt(cur_p)}</div>
        ...
        <div class="trade-title">🎯 초단타(스캘핑) 타점 설정</div>
        <div class="trade-grid">
            <div class="trade-box">
                <span>{'진입 확정가' if in_position else '예상 진입가'}</span>
                <b class="entry">{curr_symbol}{fmt(entry)}</b>
            </div>
            <div class="trade-box">
                <span>손절가 (1M/3M)</span>
                <b class="stop">{curr_symbol}{fmt(stop)} <small style="font-size:0.65rem;">({stop_pct:.1f}%)</small></b>
            </div>
            <div class="trade-box">
                <span>1차 익절 (5M)</span>
                <b class="target">{curr_symbol}{fmt(t1)} <small style="font-size:0.65rem;">(+{t1_pct:.1f}%)</small></b>
            </div>
        </div>
        <div class="trade-grid" style="margin-top:6px;">
            <div class="trade-box" style="grid-column: span 3;">
                <span>2차 익절 목표 (15M 파동 상단)</span>
                <b class="target" style="color:#6ee79a;">{curr_symbol}{fmt(t2)} <small style="font-size:0.65rem;">(+{t2_pct:.1f}%)</small></b>
            </div>
        </div>
    </div>
    """

    # 작성해주신 render_compact_html 함수를 호출하여 화면에 그립니다.
    render_compact_html(html_markup)
...
def issue_access_token(app_key, app_secret):
    """토큰을 하루 1회만 발급받고 kis_token.json 파일에 저장하여 재사용합니다."""
    token_file = "kis_token.json"

    # 1. 파일에 저장된 기존 토큰이 있는지 확인 (서버 재시작 대비)
    if os.path.exists(token_file):
        try:
            ...
            expired_time = datetime.strptime(
                saved_data.get("expired"), "%Y-%m-%d %H:%M:%S"
            )
            # 만료시간 1시간 전까지는 파일 토큰 재사용
            if datetime.now() < expired_time - timedelta(hours=1):
                return saved_data.get("access_token")
        except Exception:
            pass

    # 2. 저장된 토큰이 없거나 만료되었을 때만 1회 API 호출
    response = HTTP.post(
    ...
    if response.status_code != 200:
        raise RuntimeError(
            f"접근토큰 발급 실패: HTTP {response.status_code}"
        )
    ...
    # 3. 받아온 토큰과 만료시간을 파일에 저장
    expired_str = data.get(
    ...
    try:
        with open(token_file, "w") as f:
            json.dump({"access_token": token, "expired": expired_str}, f)
    except Exception:
        pass
