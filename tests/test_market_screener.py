...
def test_merge_rankings_prioritizes_symbols_in_both_rankings():
    ...
    assert candidates[1].screen_score == 35


def test_merge_rankings_keeps_us_leveraged_and_inverse_products():
    rankings = {
        "거래량 TOP100": [{"symb": "SOXL", "ovrs_item_name": "DIREXION SEMICONDUCTOR BULL 3X", "last": "20.1"}],
        "거래대금 TOP100": [{"symb": "SOXL", "ovrs_item_name": "DIREXION SEMICONDUCTOR BULL 3X", "rate": "4.0"}],
    }
...
def test_merge_rankings_keeps_plain_us_stock():
    rankings = {"거래량 TOP100": [{"symb": "NVDA", "ovrs_item_name": "NVIDIA", "last": "120", "rate": "4.0"}]}
    candidates = merge_rankings(Market.US, rankings)
